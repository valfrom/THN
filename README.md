...
- The control loop runs every second. Compressor state changes always respect the minimum runtime
  and restart delay defined in `main/controller/Compressor.h`.
...
```
main/
  controller/         # Low-level compressor, fan, sensor, and HVAC logic
  interface/          # ESP8266 web server, REST endpoints, and HTML template
  logging/            # Temperature and power rolling logs
  scheduler/          # Weekday/weekend schedule calculations
