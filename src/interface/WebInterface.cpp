...
void WebInterface::begin() {
  if (!LittleFS.begin()) {
    Serial.println(F("Failed to mount LittleFS; continuing without filesystem."));
  }
  registerRoutes();
  server_.begin();
...
void WebInterface::serveIndex() {
  ...
  static const char kFallback[] PROGMEM =
      "<!DOCTYPE html><html><head><meta charset='utf-8'><title>HVAC Controller</title></head>"
      "<body><h1>HVAC Controller</h1><p>Upload data/index.html via the Arduino IDE "
      "Data Upload tool to enable the full interface.</p></body></html>";
  server_.send_P(200, "text/html", kFallback);
}
